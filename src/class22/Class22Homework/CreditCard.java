package class22.Class22Homework;

public class CreditCard {
    /*
    Create a class CreditCard and define variable balance and interest. Create an instance method that will calculate interest based on the given balance.
    Create 2 subclasses: Visa and AX. In AX class override method calculate interest.
    Call the method by creating an object of each of the three classes.
    */

    int balance;
    int interest;

    void interestCalculation() {
        System.out.println((balance * interest) / 100);
    }
}

class Visa extends CreditCard {

}

class AX extends CreditCard {
    void interestCalculation() {
        System.out.println(((balance * interest) / 100) + 2);
    }
}

class CreditCardTesting{
    public static void main(String[] args) {

        CreditCard newCD = new CreditCard();
        newCD.interest = 9;
        newCD.balance = 3000;
        newCD.interestCalculation();

        Visa newVisa = new Visa();
        newVisa.interest = 3;
        newVisa.balance = 10000;
        newVisa.interestCalculation();

        AX newAX = new AX();
        newAX.interest = 2;
        newAX.balance = 2000000;
        newAX.interestCalculation();
    }
}